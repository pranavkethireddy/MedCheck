...
| GET | `/api/caregiver/my-code?userId=...` | This user's own stable caregiver access code (created on first call) |
| POST | `/api/caregiver/link` | Redeem a patient's access code, creating a caregiver_links row |
| GET | `/api/caregiver/patients?callerId=...` | Every patient the logged-in caregiver has access to |
| GET | `/api/caregiver/patient-medications?callerId=...&patientId=...` | A linked patient's medications — 403s if no caregiver_links row grants access |
| POST | `/api/caregiver/log-medication-taken` | Caregiver logs that a patient's dose was taken (caregiver-side only) |
| GET | `/api/caregiver/medication-log?callerId=...&patientId=...` | A patient's dose-taken log, newest first |
| POST | `/api/voice/speak` | Text -> mp3 audio bytes (ElevenLabs text-to-speech) |
| POST | `/api/voice/transcribe` | Recorded audio (multipart `audio` file) -> transcript text (ElevenLabs speech-to-text) |
| POST | `/api/voice/assistant` | Transcript + caregiver context -> `{ action, targetPatientName, medicationName, code, responseText }` (Gemini-interpreted) |
...
The frontend's "Continue with Google" button (LoginScreen.jsx) calls
`supabase.auth.signInWithOAuth({ provider: 'google' })` — Supabase handles
the actual OAuth exchange, but two things need to be configured outside
this codebase before it works:

1. **Google Cloud Console**: create (or reuse) a project at
   console.cloud.google.com → APIs & Services → Credentials → **Create
   Credentials → OAuth client ID** → Application type **Web application**.
   - Authorized redirect URI: `https://<your-project-ref>.supabase.co/auth/v1/callback`
     (find `<your-project-ref>` in your Supabase project URL — it's also
     shown on Supabase's Google provider settings page below).
   - Copy the resulting **Client ID** and **Client secret**.
2. **Supabase dashboard**: Authentication → Providers → **Google** → toggle
   it on, paste in the Client ID + secret from step 1, save.

That's it — no `.env` changes and no frontend code changes needed once
both of those are set. Until they are, the button stays visibly disabled
(with an explanatory tooltip) rather than silently failing, and a failed
attempt (e.g. testing before the provider is fully enabled) surfaces
Supabase's actual error message back on the login screen instead of
failing silently.

## Two Gemini keys (quota isolation)

Google's Gemini free tier rate-limits **per API key** (~10 requests/minute),
not per app. Interaction explanations, the risk summary, and the caregiver
voice assistant all use Gemini — if they all share one key, a burst of
testing on one feature can rate-limit the others, even though they're
unrelated. This app splits them across two independent keys so that
doesn't happen:

- `GEMINI_API_KEY` — interaction explanations + the risk summary.
- `GEMINI_VOICE_API_KEY` — the caregiver voice assistant only.

**Get two keys from two different Google accounts** (e.g. two teammates
each grab their own at aistudio.google.com/apikey) — one key per Google
account gets its own separate free quota, so this genuinely doubles your
usable Gemini throughput rather than just moving the same limit around.
Put both in `.env`; see `.env.example` for exactly where.

If `GEMINI_VOICE_API_KEY` isn't set, voice quietly falls back to sharing
`GEMINI_API_KEY` — nothing breaks, you just don't get the quota-isolation
benefit until the second key is added. Each key/pool also has its own
independent rate-limit cooldown (see `app/gemini_client.py`'s circuit
breaker) — the frontend's app-wide AI banner reflects the `GEMINI_API_KEY`
pool, and the voice assistant's own mic-cooldown state reflects
`GEMINI_VOICE_API_KEY`'s pool, so a rate limit on one no longer greys out
the other in the UI.

**On the model name:** both pools default to `gemini-flash-latest` (Google's
own alias for their current recommended flash-tier model), deliberately
*not* a specific dated model like `gemini-2.5-flash`. We hit that exact
model live on a newly-created key — Gemini returned a 404 saying "This
model models/gemini-2.5-flash is no longer available to new users," even
though it still showed up in `ListModels` and still worked fine for an
older, grandfathered key. If a key ever 404s on a model again, the error
message we surface now includes Gemini's own explanation verbatim (it
usually names the exact replacement), and `GEMINI_VOICE_MODEL` /
`GEMINI_MODEL` let you override the model per pool without touching code.
