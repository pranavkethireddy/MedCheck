...
def main():
    ...
    def _t4e():
        ...
        _gc._cache.clear()
        _gc._pending.clear()
        _gc._circuit_open_until = {"default": 0.0, "voice": 0.0}
    ...
    def _t4g():
        # Regression test for the two-key quota isolation feature: the
        # caregiver voice assistant uses its own Gemini key/quota (the
        # "voice" pool) separate from interaction explanations/risk summary
        # (the "default" pool) specifically so a rate limit on one doesn't
        # block the other. If tripping "default" also blocked "voice" (or
        # vice versa), that isolation would be worthless.
        _gc._circuit_open_until = {"default": 0.0, "voice": 0.0}
        _gc._trip_circuit("default")
        assert _gc._circuit_is_open("default"), "default pool should be open right after tripping it"
        assert not _gc._circuit_is_open("voice"), "voice pool must stay unaffected by the default pool tripping"

        _gc._circuit_open_until = {"default": 0.0, "voice": 0.0}
        _gc._trip_circuit("voice")
        assert _gc._circuit_is_open("voice"), "voice pool should be open right after tripping it"
        assert not _gc._circuit_is_open("default"), "default pool must stay unaffected by the voice pool tripping"

        _gc._circuit_open_until = {"default": 0.0, "voice": 0.0}

    check("gemini_client's two Gemini key pools ('default' and 'voice') have independent circuit breakers", _t4g)

    def _t4h():
        # call_gemini(pool="voice") should read GEMINI_VOICE_API_KEY, not
        # GEMINI_API_KEY -- the whole point of the split.
        seen_keys = []

        def fake_urlopen(req, timeout=10):
            seen_keys.append(req.headers.get("X-goog-api-key"))
            raise _urllib_error.HTTPError("http://x", 500, "boom", None, None)

        real_urlopen = _gc.urllib.request.urlopen
        _gc.urllib.request.urlopen = fake_urlopen
        old_default = os.environ.get("GEMINI_API_KEY")
        old_voice = os.environ.get("GEMINI_VOICE_API_KEY")
        os.environ["GEMINI_API_KEY"] = "default-key-123"
        os.environ["GEMINI_VOICE_API_KEY"] = "voice-key-456"
        try:
            try:
                _gc.call_gemini("hi", pool="voice")
            except _urllib_error.HTTPError:
                pass
            try:
                _gc.call_gemini("hi", pool="default")
            except _urllib_error.HTTPError:
                pass
            assert seen_keys == ["voice-key-456", "default-key-123"], seen_keys
        finally:
            _gc.urllib.request.urlopen = real_urlopen
            for name, old in [("GEMINI_API_KEY", old_default), ("GEMINI_VOICE_API_KEY", old_voice)]:
                if old is not None:
                    os.environ[name] = old
                else:
                    os.environ.pop(name, None)

    check("call_gemini(pool='voice') uses GEMINI_VOICE_API_KEY, pool='default' uses GEMINI_API_KEY", _t4h)

    def _t4i():
        # Regression test for the live bug reported: a second Gemini key
        # can get a 404 on generateContent even for a model ListModels
        # shows it CAN see (turned out to be Google cutting off a specific
        # dated model -- gemini-2.5-flash -- for any newly-created key,
        # while an older/grandfathered key kept working). The raw urllib
        # HTTPError must never leak through as a cryptic "HTTP Error 404:
        # Not Found" -- call_gemini must turn it into an actionable
        # RuntimeError naming the model and the env var, and must NOT retry
        # (retrying the same rejected model is pointless, unlike a 429).
        # This variant has no response body (fp=None) -- exercises the
        # fallback path when Gemini's own error text can't be read.
        def fake_urlopen(req, timeout=10):
            raise _urllib_error.HTTPError("http://x", 404, "Not Found", None, None)

        real_urlopen = _gc.urllib.request.urlopen
        _gc.urllib.request.urlopen = fake_urlopen
        old_key = os.environ.get("GEMINI_VOICE_API_KEY")
        os.environ["GEMINI_VOICE_API_KEY"] = "some-key"
        try:
            try:
                _gc.call_gemini("hi", pool="voice")
                ...
                assert "GEMINI_VOICE_API_KEY" in msg, msg
                assert "GEMINI_VOICE_MODEL" in msg, msg
            except _urllib_error.HTTPError:
                raise AssertionError("raw HTTPError leaked through instead of an actionable RuntimeError")
        finally:
            _gc.urllib.request.urlopen = real_urlopen
            if old_key is not None:
                os.environ["GEMINI_VOICE_API_KEY"] = old_key
            else:
                os.environ.pop("GEMINI_VOICE_API_KEY", None)

    check("call_gemini turns a 404 (model not found for this key) into an actionable error, not a raw HTTPError", _t4i)

    def _t4j():
        # Same scenario, but with a real Gemini-shaped error body attached
        # (the actual live response: "This model models/gemini-2.5-flash is
        # no longer available to new users...") -- confirms call_gemini
        # surfaces GEMINI'S OWN message verbatim rather than only our
        # generic wrapper text, since Gemini's own wording is consistently
        # more specific (it often names the exact replacement model) than
        # anything we could infer from the outside.
        import io as _io
        ...
        def fake_urlopen(req, timeout=10):
            raise _urllib_error.HTTPError("http://x", 404, "Not Found", None, _io.BytesIO(body))
        ...
        try:
            try:
                ...
                msg = str(e)
                assert "no longer available to new users" in msg, msg
                assert "gemini-3.6-flash" in msg, msg
        finally:
            _gc.urllib.request.urlopen = real_urlopen
            if old_key is not None:
                os.environ["GEMINI_VOICE_API_KEY"] = old_key
    ...
    check("call_gemini surfaces Gemini's own 404 error message verbatim (e.g. naming a replacement model)", _t4j)
