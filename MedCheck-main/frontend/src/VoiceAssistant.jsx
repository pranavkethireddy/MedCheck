...
function VoiceAssistant({
  ...
  // Reads the "voice" pool's circuit breaker specifically (its own Gemini
  // key/quota — see backend/app/gemini_client.py and voice_assistant.py),
  // NOT the same "default" one AiStatusBanner.jsx shows app-wide for
  // interaction explanations/risk summary. The two keys are now
  // independent, so a burst of interaction checks rate-limiting the
  // default key shouldn't grey out the mic, and vice versa. This still
  // greys the mic out and shows a live countdown *before* someone can even
  ...
  const { limited: aiLimited, retryAfterSeconds } = useAiStatus('voice')
